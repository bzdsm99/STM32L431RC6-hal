...
void usart_init(uint32_t baudrate)
{
    ...
    HAL_UART_Receive_IT(&g_uart_handle, rx_buffer, 1);
#endif
}
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    if (huart->Instance == USART_UX)                    /* 如果是设定的串口 */
    {
        ...
        HAL_UART_Receive_IT(&g_uart_handle, (uint8_t *)g_rx_buffer, 1);
#else
        /* 不使用DMA时，从中断接收缓冲区获取数据 */
        static uint8_t rx_buffer[1];
        if (g_uart_rx_buf.count < UART_RX_BUFFER_SIZE) {
            g_uart_rx_buf.buffer[g_uart_rx_buf.head] = rx_buffer[0];
            g_uart_rx_buf.head = (g_uart_rx_buf.head + 1) % UART_RX_BUFFER_SIZE;
            g_uart_rx_buf.count++;
        }
        /* 重新启动中断接收 */
...
void HAL_UART_ErrorCallback(UART_HandleTypeDef *huart)
{
    if(huart->Instance == USART_UX)
    {
        ...
        if (huart->hdmarx != NULL) {
            ...
            HAL_UART_Receive_DMA(huart, g_rx_buffer, DMA_RX_BUFFER_SIZE);
        }
#else
        /* 非DMA模式，重启中断接收 */
        static uint8_t interrupt_buffer[1];
        HAL_UART_Receive_IT(&g_uart_handle, interrupt_buffer, 1);
#endif
    }
}
 ...
 * @retval      1: 成功读取一行数据, 0: 没有完整的行数据
 */
bool uart_read_line(char* buffer, uint16_t buffer_size)
{
    uint16_t i, j;
    uint16_t count = g_uart_rx_buf.count;
    ...
    for (i = 0; i < count && i < UART_RX_BUFFER_SIZE - 1; i++) {
        // 查找 \r\n 序列
        if (temp_buffer[i] == '\r' && (i + 1 < count) && temp_buffer[i + 1] == '\n') {
            // 复制数据到目标缓冲区（不包括 \r\n）
            uint16_t copy_len = (i < buffer_size - 1) ? i : buffer_size - 1;
            for (j = 0; j < copy_len; j++) {
        ...
        // 也处理单独的 \r 或 \n 的情况
        else if (temp_buffer[i] == '\r' || temp_buffer[i] == '\n') {
            // 复制数据到目标缓冲区（不包括结束符）
            uint16_t copy_len = (i < buffer_size - 1) ? i : buffer_size - 1;
            for (j = 0; j < copy_len; j++) {
                buffer[j] = temp_buffer[j];
